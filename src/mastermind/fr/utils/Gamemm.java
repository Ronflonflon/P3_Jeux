	...
	public int challenger_mm(int dev) {
		...
		try {
			...
			while (win != 1) {
				...
				System.out.println("Taille personne : " + my_person.size_try + " et taille ordi : " + my_computer.size_to_guess);
				if (my_person.size_try == my_computer.size_to_guess) {
					compar.challenger_tab_compar(my_person.tab_try, my_computer.tab_to_guess);
					my_person.shots++;
				} else {
					messages.fail_value();
				}
